...
def _show_error(text: str, title: str = "TG WS Proxy — Ошибка"):
    _show_dialog(text, title, kind="error")


def _show_info(text: str, title: str = "TG WS Proxy"):
    _show_dialog(text, title, kind="info")
...
def _on_open_in_telegram(icon=None, item=None):
    ...
    try:
        ...
        if result.returncode != 0:
            raise RuntimeError(f"open returned {result.returncode}")
    except Exception:
        log.info("open failed, copying to clipboard")
        try:
            pyperclip.copy(url)
            _show_info(
                f"Не удалось открыть Telegram автоматически.\n\n"
                f"Ссылка скопирована в буфер обмена, отправьте её в Telegram и нажмите по ней:\n{url}",
                "TG WS Proxy")
        except Exception as exc:
...
def _edit_config_dialog():
    ...
    dc_textbox = ctk.CTkTextbox(frame, width=370, height=120,
                                font=("Menlo", 12), corner_radius=10,
                                fg_color=FIELD_BG, border_color=FIELD_BORDER,
                                border_width=1, text_color=TEXT_PRIMARY)
...
def _show_first_run():
    ...
    ctk.CTkLabel(title_frame, text="Прокси запущен и работает в строке меню",
    ...
    sections = [
        ...
        ("  Автоматически:", True),
        (f"  Нажмите на иконку в строке меню → «Открыть в Telegram»", False),
...
def _show_ipv6_dialog():
    _show_info(
        "На вашем Mac включена поддержка подключения по IPv6.\n\n"
...
def run_tray():
    ...
    log.info("Menu bar icon running")
    _tray_icon.run()
...
def main():
    if not _acquire_lock():
        _show_info("Приложение уже запущено.", "TG WS Proxy")
        return
